...
{
    market.bind_callback([](const OrderBookData &tick)
    {
        ...
        print_row("exchange_id", tick.exchange_id);
        print_row("last_price", tick.last_price);
        print_row("qty", tick.qty);
        print_row("turnover", tick.turnover);
        print_row("trades_count", tick.trades_count);
        print_row("data_time", tick.data_time);
        std::cout << "+----------------------+------------------------------+\n";
        ...
        for (int i = 0; i < 10; ++i)
        {
            std::cout << "| " << std::left << std::setw(5) << (i + 1)
                    << " | " << std::right << std::setw(13) << tick.bid[i]
                    << " | " << std::setw(10) << tick.bid_qty[i]
                    << " | " << std::setw(13) << tick.ask[i]
                    << " | " << std::setw(10) << tick.ask_qty[i]
                    << " |\n";
        }
        std::cout << "+-------+---------------+------------+---------------+------------+\n";
    });
...
{
    market.bind_callback([](const OrderBookData &)
            {});
}
