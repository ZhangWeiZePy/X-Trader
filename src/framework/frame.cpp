...
orderref_t frame::insert_order(const stratid_t sid, eOrderFlag order_flag, const std::string &contract,
                               eDirOffset dir_offset, double price, int volume)
{
...
void frame::register_bar_receiver(const std::string &contract, uint32_t period, bar_receiver *receiver)
{
    ...
    {
        _resample[contract][period] = std::make_shared<
            resample>(period, _realtime->get_instrument(contract).price_tick);
    }
...
void frame::run_until_close(const std::vector<std::shared_ptr<strategy> > &strategys)
{
    ...
    if (!_realtime->init(_filename, _contracts))
    {
        return;
    }
