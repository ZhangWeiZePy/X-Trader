...
orderref_t realtime::insert_order(eOrderFlag order_flag, const std::string &contract, eDirOffset dir_offset,
        ...
        if (_instrument_map[order.instrument_id].is_use_history)
        {
            switch (order.dir_offset)
            {
                case eDirOffset::BuyOpen:
                    p.long_.open_no_trade += volume_total;
                    break;
                case eDirOffset::SellOpen:
                    p.short_.open_no_trade += volume_total;
                    break;
...
void realtime::handle_trade(const Order &order)
{
        ...
        if (_instrument_map[order.instrument_id].is_use_history)
        {
            switch (order.dir_offset)
            {
                case eDirOffset::BuyOpen:
                    ...
                    p.long_.avg_open_cost = (p.long_.avg_open_cost * (p.long_.position - vol_traded_once) + order.
                                             limit_price * vol_traded_once) / p.long_.position;
                    p.long_.avg_posi_cost = (p.long_.avg_posi_cost * (p.long_.position - vol_traded_once) + order.
                                             limit_price * vol_traded_once) / p.long_.position;
                    break;
                case eDirOffset::SellOpen:
                    ...
                    p.short_.avg_open_cost = (p.short_.avg_open_cost * (p.short_.position - vol_traded_once) + order.
                                              limit_price * vol_traded_once) / p.short_.position;
                    p.short_.avg_posi_cost = (p.short_.avg_posi_cost * (p.short_.position - vol_traded_once) + order.
                                              limit_price * vol_traded_once) / p.short_.position;
                    break;
                case eDirOffset::BuyCloseYesterday:
                case eDirOffset::BuyClose:
    ...
    if (order.order_status == eOrderStatus::AllTraded)
    {
        ...
        if (it_o != _order_map.end())
        {
            _order_map.erase(it_o);
        }
    ...
    if (_order_event.on_trade)
    {
        _order_event.on_trade(order);
    }
}
...
void realtime::handle_cancel(const Order &order)
{
    ...
    if (iter != _order_map.end())
    {
        _order_map.erase(iter);
    }

    if (_order_event.on_cancel)
    {
        _order_event.on_cancel(order);
    }
...
void realtime::handle_error(const Order &order)
{
    ...
    if (_order_event.on_error)
    {
        _order_event.on_error(order);
    }
}
