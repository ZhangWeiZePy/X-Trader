...
inline void send2wecom(const char *text)
{
    ...
    std::string param =
            R"( -H "Content-Type: application/json" -d "{\"msgtype\": \"text\",\"text\": {\"content\": \"%s\"}}")";
...
inline const char *reqRtnReason(int rtn)
{
    switch (rtn)
    {
        ...
        case -1: ///因网络原因发送失败
            ...
            break;
        case -2: ///未处理请求队列总数量超限
            return "Unprocessed requests exceeded the allowed volume";
            break;
        case -3: ///每秒发送请求数量超限
            return "The number of requests sent per second exceeds the allowed volume";
            break;
        default: ///未知
            return "Unknown";
    }
