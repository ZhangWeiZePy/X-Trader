...
struct __attribute__((aligned(64))) Tape
{
    ...
    Tape() :
        last_volume(0), last_open_interest(.0F), tape_dir(eTapeDir::Flat)
    {}
...
struct __attribute__((aligned(64))) Sample
{
    ...
    //获取不平衡订单
    std::pair<std::shared_ptr<std::vector<double> >, std::shared_ptr<std::vector<double> > > get_imbalance(
    ...
    Sample() :
        time(""), period(0U), open(0), close(0), high(0), low(0), volume(0), delta(0), poc(.0), price_tick(1.0)
    {}
    ...
    void clear()
    {
        ...
        delta = 0;
        poc = 0;
        active_buy_volume.clear();
        active_sell_volume.clear();
    }
};
