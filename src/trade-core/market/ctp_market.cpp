...
void ctp_market::OnFrontDisconnected(int nReason)
{
    printf("nReason=%d\n", nReason);
    char text[128];
    sprintf(text, "%s Market Front Disconnected", _user_id.c_str());
    //send2wecom(text);
}

///登录请求响应
void ctp_market::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo,
...
///登出请求响应
void ctp_market::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout, CThostFtdcRspInfoField *pRspInfo,
                                 int nRequestID, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
...
///订阅行情应答
void ctp_market::OnRspSubMarketData(CThostFtdcSpecificInstrumentField *pSpecificInstrument,
                                    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    if (bIsLast && !_is_ready.load())
    {
        ...
        char text[128];
        sprintf(text, "md@%s is ready!", _user_id.c_str());
        //send2wecom(text);
...
///取消订阅行情应答
void ctp_market::OnRspUnSubMarketData(CThostFtdcSpecificInstrumentField *pSpecificInstrument,
                                      CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
...
void ctp_market::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *ptr)
{
    ...
    _tick.bid_price[0] = ptr->BidPrice1;
    _tick.bid_volume[0] = ptr->BidVolume1;
    _tick.bid_price[1] = ptr->BidPrice2;
    _tick.bid_volume[1] = ptr->BidVolume2;
    _tick.bid_price[2] = ptr->BidPrice3;
    _tick.bid_volume[2] = ptr->BidVolume3;
    _tick.bid_price[3] = ptr->BidPrice4;
    _tick.bid_volume[3] = ptr->BidVolume4;
    _tick.bid_price[4] = ptr->BidPrice5;
    _tick.bid_volume[4] = ptr->BidVolume5;

    _tick.ask_price[0] = ptr->AskPrice1;
    _tick.ask_volume[0] = ptr->AskVolume1;
    _tick.ask_price[1] = ptr->AskPrice2;
    _tick.ask_volume[1] = ptr->AskVolume2;
    _tick.ask_price[2] = ptr->AskPrice3;
    _tick.ask_volume[2] = ptr->AskVolume3;
    _tick.ask_price[3] = ptr->AskPrice4;
    _tick.ask_volume[3] = ptr->AskVolume4;
    _tick.ask_price[4] = ptr->AskPrice5;
    _tick.ask_volume[4] = ptr->AskVolume5;
