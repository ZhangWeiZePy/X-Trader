...
orderref_t ctp_trader::insert_order(eOrderFlag order_flag, const std::string &contract, eDirOffset dir_offset,
...
{
    char text[128];
    sprintf(text, "%s Trade Front Disconnected", _user_id.c_str());
    //send2wecom(text);
}

///客户端认证响应
void ctp_trader::OnRspAuthenticate(CThostFtdcRspAuthenticateField *pRspAuthenticateField,
                                   CThostFtdcRspInfoField *pRspInfo, int nRequorder_id, bool bIsLast)
{
...
///登录请求响应
void ctp_trader::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo,
                                int nRequorder_id, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
        return;
...
///登出请求响应
void ctp_trader::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout, CThostFtdcRspInfoField *pRspInfo,
                                 int nRequorder_id, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
...
///投资者结算结果确认响应
void ctp_trader::OnRspSettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm,
                                            CThostFtdcRspInfoField *pRspInfo, int nRequorder_id, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
...
///请求查询合约响应
void ctp_trader::OnRspQryInstrument(CThostFtdcInstrumentField *ptr, CThostFtdcRspInfoField *pRspInfo, int nRequorder_id,
                                    bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
...
///请求查询投资者持仓响应
void ctp_trader::OnRspQryInvestorPosition(CThostFtdcInvestorPositionField *ptr, CThostFtdcRspInfoField *pRspInfo,
                                          int nRequorder_id, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
    ...
    if (ptr)
    {
        ...
        if (ptr->Position)
        {
            ...
            posi.avg_open_cost = (posi.avg_open_cost * (posi.position - ptr->Position) * volume_multiple + ptr->
                                  OpenCost)
                                 / (posi.position * volume_multiple);

            posi.avg_posi_cost = (posi.avg_posi_cost * (posi.position - ptr->Position) * volume_multiple + ptr->
                                  PositionCost)
                                 / (posi.position * volume_multiple);
        }
    }
...
///请求查询资金账户响应
void ctp_trader::OnRspQryTradingAccount(CThostFtdcTradingAccountField *pAcc, CThostFtdcRspInfoField *pRspInfo,
                                        int nRequorder_id, bool bIsLast)
{
    if (pRspInfo && pRspInfo->ErrorID)
    {
...
///请求查询报单响应
void ctp_trader::OnRspQryOrder(CThostFtdcOrderField *pOrder, CThostFtdcRspInfoField *pRspInfo, int nRequorder_id,
                               bool bIsLast)
{
    ...
    if (bIsLast && !_is_ready.load())
    {
        ...
        char text[128];
        sprintf(text, "td@%s is ready!", _user_id.c_str());
        //send2wecom(text);
...
///报单录入请求响应
void ctp_trader::OnRspOrderInsert(CThostFtdcInputOrderField *pInputOrder, CThostFtdcRspInfoField *pRspInfo,
                                  int nRequorder_id, bool bIsLast)
{
...
///报单操作请求响应
void ctp_trader::OnRspOrderAction(CThostFtdcInputOrderActionField *pInputOrderAction, CThostFtdcRspInfoField *pRspInfo,
                                  int nRequorder_id, bool bIsLast)
{
...
void ctp_trader::OnRtnOrder(CThostFtdcOrderField *pOrder)
{
    ...
    eOrderSubmitStatus order_submit_status;
    to_local_orderSubmitStatus(pOrder->OrderSubmitStatus, order_submit_status);
    eOrderStatus order_status;
    to_local_orderStatus(pOrder->OrderStatus, order_status);

    Order &order = _order_map[order_ref];
    if (order.session_id)
    {
        if (order.order_status == order_status && order.volume_total == pOrder->VolumeTotal)
        {
            return;
        }
    } else
