...
private:
    ...
    inline uint32_t generate_reqid()
    {
        return _req_id.fetch_add(1);
    }
...
public:
    ...
    virtual void OnRspAuthenticate(CThostFtdcRspAuthenticateField *pRspAuthenticateField,
                                   CThostFtdcRspInfoField *pRspInfo, int nRequorder_id, bool bIsLast) override;

    ///登录请求响应
    virtual void OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo,
                                int nRequorder_id, bool bIsLast) override;
    ...
    ///请求查询报单响应
    virtual void OnRspQryOrder(CThostFtdcOrderField *pOrder, CThostFtdcRspInfoField *pRspInfo, int nRequorder_id,
    ...
    inline void to_local_orderSubmitStatus(const char &ctp, eOrderSubmitStatus &local)
    {
        switch (ctp)
        {
            case THOST_FTDC_OSS_InsertSubmitted: ///已经提交
                local = eOrderSubmitStatus::InsertSubmitted;
                break;
            case THOST_FTDC_OSS_CancelSubmitted: ///撤单已经提交
                local = eOrderSubmitStatus::CancelSubmitted;
                break;
            case THOST_FTDC_OSS_ModifySubmitted: ///修改已经提交
                local = eOrderSubmitStatus::ModifySubmitted;
                break;
            case THOST_FTDC_OSS_Accepted: ///已经接受
                local = eOrderSubmitStatus::Accepted;
                break;
            case THOST_FTDC_OSS_InsertRejected: ///报单已经被拒绝
                local = eOrderSubmitStatus::InsertRejected;
                break;
            case THOST_FTDC_OSS_CancelRejected: ///撤单已经被拒绝
                local = eOrderSubmitStatus::CancelRejected;
                break;
            case THOST_FTDC_OSS_ModifyRejected: ///改单已经被拒绝
                local = eOrderSubmitStatus::ModifyRejected;
                break;
        }
    }
