...
struct XTPOrderInfoEx
{
    ...
    ///报单类型
    TXTPOrderTypeType       order_type;
    ///报单编号 --交易所单号，深交所有此字段，上交所股票债券业务有此字段，上交所申赎业务暂无此字段
...
struct XTPUserTerminalInfoReqEx {
	...
	XTPVersionType  client_version;						///<客户端版本号
    char  macos_sno[XTP_MACOS_SNO_LEN];					///<MacOS系统的序列号，仅为MacOS系统需要填写
    char  imei[XTP_IMEI_LEN];                           ///<移动Android端标识符
    char  idfv[XTP_IDFV_LEN];                           ///<移动ios端标识符
    char  guid[XTP_GUID_LEN];                           ///<移动其他终端标识符
    char  regist_tel[XTP_TEL_LEN];                      ///<注册手机号码
    char  used_tel[XTP_TEL_LEN];                        ///<实际使用手机号码
	char  unused[30];									///<预留
};
